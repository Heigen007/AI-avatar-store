...
// const db: IDbInterface = getDataBase() // Инициализация параметров для подключения к БД
...
export class DatabaseConnection {
    ...
    private _currentTransactionId = 0
    ...
    public async rollback(): Promise<void> {
        try {
            ...
            this.isTransactionStarted = false
            logger.debug('DatabaseConnection: rollback: Success (transaction_id=' + this._currentTransactionId + ')')
            this._currentTransactionId = 0
        }
        catch (error: unknown) {
            const errMsg = error instanceof Error ? error.message : String(error)
